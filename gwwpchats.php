...
/*
 ...
 Plugin URI: 
 Description: Adds essaging buttons to profiles.
 Author: GippslandWeb
 Version: 1.0
...
if(!is_user_logged_in())
    ...
    if(get_current_user_id() != bp_displayed_user_id())
        echo('<div class="message-button  generic-button" id="message-button"><a href="http://localhost/wpc-messages/'.bp_core_get_username(bp_displayed_user_id()).'/">Message</a></div>');
    else
        {
            $cnt = $this->unread_counter(get_current_user_id());
            if($cnt > 0)
                echo('<div class="message-button  generic-button" id="message-button"><a href="http://localhost/wpc-messages/">'.$cnt.'New Message</a></div>');
            else
                    echo('<div class="message-button  generic-button" id="message-button"><a href="http://localhost/wpc-messages/">View Message</a></div>');
